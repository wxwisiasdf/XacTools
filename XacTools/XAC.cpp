...
namespace ogl {
	...
	const char* parse_xac_mesh_v1(xac_context& context, const char* start, const char* end, parsers::error_handler& err) {
		...
		if (cd.node_id >= context.nodes.size()) {
...
#endif
			for (uint32_t i = 0; i < cd.num_vertices; ++i) {
				switch (xac_vertex_block_v1_type(vbh.ident)) {
				case xac_vertex_block_v1_type::normal:
					if (vbh.size != sizeof(xac_vector3f)) {
						err.accumulated_errors += "Attribute size doesn't match! [normal] (" + context.filename + ")\n";
						start += vbh.size;
					}
					else {
				...
				case xac_vertex_block_v1_type::vertex:
					if (vbh.size != sizeof(xac_vector3f)) {
						err.accumulated_errors += "Attribute size doesn't match! [vertex] (" + context.filename + ")\n";
						start += vbh.size;
					}
					else {
				...
				case xac_vertex_block_v1_type::texcoord:
					if (vbh.size != sizeof(xac_vector2f)) {
						err.accumulated_errors += "Attribute size doesn't match! [texcoord] (" + context.filename + ")\n";
						start += vbh.size;
					}
					else {
				...
				case xac_vertex_block_v1_type::weight:
					if (vbh.size != sizeof(xac_vector4f)) {
						err.accumulated_errors += "Attribute size doesn't match! [weight] (" + context.filename + ")\n";
						start += vbh.size;
					}
					else {
				...
				case xac_vertex_block_v1_type::influence_indices:
					if (vbh.size != sizeof(uint32_t)) {
						err.accumulated_errors += "Attribute size doesn't match! [influenceRange] (" + context.filename + ")\n";
						start += vbh.size;
					}
					else {
		...
		if (cd.is_collision_mesh) {
			...
			assert(node.meshes.size() < std::numeric_limits<decltype(node.collision_mesh)>::max());
			...
			assert(node.meshes.size() < std::numeric_limits<decltype(node.visual_mesh)>::max());
	...
	void finish(xac_context& context) {
		...
		for (auto& node : context.nodes) {
			for (auto& o : node.meshes) {
				const ogl::xac_vector4f q = node.rotation;
				const ogl::xac_mat4x4 qm{
				...
				for (uint32_t i = 0; i < o.vertices.size(); i++) {
					ogl::xac_vector3f v = o.vertices[i];
	...
	std::string get_canonical_material_name(xac_pp_actor_material const& mat) {
		if (!mat.name.empty())
			return mat.name;
		float total = 0.f;
		total += mat.ambient_color.r;
		total += mat.ambient_color.g;
		total += mat.ambient_color.b;
		total += mat.ambient_color.a;
		total += mat.diffuse_color.r;
		total += mat.diffuse_color.g;
		total += mat.diffuse_color.b;
		total += mat.diffuse_color.a;
		total += mat.emissive_color.r;
		total += mat.emissive_color.g;
		total += mat.emissive_color.b;
		total += mat.emissive_color.a;
		total += mat.specular_color.r;
		total += mat.specular_color.g;
		total += mat.specular_color.b;
		total += mat.specular_color.a;
		for (const auto& layer : mat.layers) {
			total += layer.amount;
			total += layer.rotation;
			total += layer.u_offset;
			total += layer.v_offset;
			total += layer.u_tiling;
			total += layer.v_tiling;
			for (const auto c : layer.texture)
				total += float(c);
		}
		total *= 1000.f;
		std::string base_str = "AnonMat_" + std::to_string(int32_t(total)) + (mat.wireframe ? "_1" : "_2");
		for (const auto& layer : mat.layers) {
			for (const auto c : layer.texture)
				if (isalnum(c))
					base_str += c;
	...
	xac_pp_actor_material_layer get_diffuse_layer(xac_pp_actor_material const& mat) {
		for (const auto& layer : mat.layers) {
			if (layer.map_type == xac_pp_material_map_type::diffuse) {
				return layer;
			}
		}
		for (const auto& layer : mat.layers) {
		...
		return xac_pp_actor_material_layer{};
	}
	std::string get_diffuse_texture(ogl::xac_context const& context, xac_pp_actor_material const& mat) {
		auto layer = get_diffuse_layer(mat);
		if (!layer.texture.empty())
			return layer.texture;
		if (file_exists(context.base_game_path + "\\gfx\\anims\\" + context.filename + "Diffuse.dds")) {
			return context.filename + "Diffuse";
		}
		return context.filename + "_diffuse";
	}
	xac_pp_actor_material_layer get_specular_layer(xac_pp_actor_material const& mat) {
		for (const auto& layer : mat.layers) {
			if (layer.map_type == xac_pp_material_map_type::specular) {
				return layer;
			}
		}
		for (const auto& layer : mat.layers) {
	...
	std::string get_specular_texture(ogl::xac_context const& context, xac_pp_actor_material const& mat) {
		auto layer = get_specular_layer(mat);
		if (!layer.texture.empty())
			return layer.texture;
		if (file_exists(context.base_game_path + "\\gfx\\anims\\" + context.filename + "Specular.dds")) {
			return context.filename + "Specular";
		}
		return context.filename + "_specular";
	}
}
